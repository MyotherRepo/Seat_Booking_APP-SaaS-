...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def seat_grid_status(request, floorplan_id):  # 👈 Accept floorplan_id from URL
    booking_date = request.query_params.get('date', date.today())

    seats = Seat.objects.filter(floor_plan_id=floorplan_id, is_active=True)
    bookings = Booking.objects.filter(seat__floor_plan_id=floorplan_id, date=booking_date)
